document.addEventListener('DOMContentLoaded', async function () {
    ...
    function createValidationTestElement(result, testNumber) {
        ...
        element.innerHTML = `
            ...
            ${result.description ? `<p class="text-sm text-slate-400 mb-2">${result.description}</p>` : ''}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div>
                    <span class="font-medium text-slate-400">Entrée:</span>
                    <pre class="mt-1 p-2 bg-slate-900/50 rounded text-slate-200 text-xs overflow-x-auto">${escapeHtml(result.input || '')}</pre>
                </div>
                <div>
